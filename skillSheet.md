...
Kintone 上でアプリを作成し、基幹システムとの連携や生成 AI の活用のために大幅なカスタマイズを行っています。バンドルツールとして Vitest を使用し、テストを Vitest で行い、GitHub Actions での CI/CD を構築しました。詳細は[こちら](https://qiita.com/miyuc/items/2c82faa5f4729f97f795)をご覧ください。また、データベースや OpenAI を使用した機能を REST API で提供。 Fly.io 上で API と定期バッチ処理を実装しています。
